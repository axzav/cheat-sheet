...
Проводит запрос через цепочку обработчиков. Обработчик решает, пройдет ли запрос дальше по цепочке или оборвется на конкретном обработчике (может быть и обратная реализация когда обработчик не может обработать запрос и тогда передает его дальше)

![](../_img/chain_of_responsibility.png)

**Пример**: в PHP распространенным примером использования паттерна является реализация HTTP middleware request handlers - https://www.php-fig.org/psr/psr-15/
Запрос проходит через цепочку обработчиков, которые проверяют права на выполнение запроса, рейт лимитер проверки, проверка пользователя и тд. В случае, если все необходимые проверки пройдены, то запрос будет передан основному обработчику
