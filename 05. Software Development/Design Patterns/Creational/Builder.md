...
Порождающий шаблон проектирования, который позволяет разбить создание сложного объекта на шаги.

В случае, когда объект имеет сложную структуру и может быть создан с разным количество параметров, это может привести к тому, что конструктор будет перегружен очень большим количеством параметров

Шаблон builder позволяет организовать процесс инстанцирования объекта только с необходимыми параметрами. Чаще всего это будет выглядеть как цепочка вызовов - `$builder->setParam1(12)->setParam3(true)->getResult()`

Билдер может быть организован как некоторое количество конкретных билдеров + директор, который может быть использован для формирования часто используемых комбинаций. Или же билдер может быть реализован как один конкретный класс.

![](../_img/builder.png)

Пример билдера с одним конкретным классом - библиотека **TcPDF**. В этой библиотеке собирается только один конкретный класс библиотеки **TcPDF*

Пример билдера с несколькими конкретными билдерами - sql query builder. Для разных баз данных может различаться синтаксис запросов. Таким образом для MySQL и Postgres потребуются 2 конкретных билдера. Директор в этом случае не требуется, т.к. сборка каждого запроса будет различаться
