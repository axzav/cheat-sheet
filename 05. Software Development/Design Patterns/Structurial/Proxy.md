...
Позволяет добавить новую функциональность методам объекта путем создания обертки над этим объектом. Новая функциональность добавляется в методы обертки до или после вызова метода оборачиваемого объекта. Также шаблон предусматривает сохранение интерфейса, таким образом оригинальный объект и прокси-объект взаимозаменяемы в клиентском коде

![](../_img/proxy.png)

**Пример**: есть класс загрузчик видео *SimpleDownloader*. Появилась потребность добавить возможность кеширования загрузки, чтобы не загружать видео вновь при повторном запросе. Добавляться класс *CachingDownloader*, который оборачивает *SimpleDownloader* и добавляет функционал кеширования методу *download*
