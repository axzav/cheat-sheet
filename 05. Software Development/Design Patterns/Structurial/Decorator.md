...
Декоратор позволяет добавлять новую функциональность посредствам классов-оберток. Шаблон позволяет создавать неограниченное количество оберток и использовать их вложенными друг в друга

![](../_img/decorator.png)

**Concrete Component** - класс, функциональность которого будет расширяться. Метод *execute* содержит базовую логику, которая будет расширена в *Conrete Decorator*
**Base Decorator** - базовый класс для *Conrete Decorator*. Содержит инфраструктуру для 
Concrete Decorator
**Concrete Decorator** - классы-обертки. Содержат в себе конкретные реализации метода *execute* с новой функциональностью.

Классы декоратора спроектированы так, что могут принимать в конструкторе как **Concrete Component**, так и **Conrete Decorator**, что позволяет организовывать вложенную структуру декораторов и использовать их в нужной комбинации

**Пример**: форматирование и фильтрация данных перед отображением на веб-странице. Например, комментарии. Может понадобится удалить некоторые HTML теги, удалить опасные теги, отфильтровать стоп слова и тд. Эту функциональность можно реализовать путем создания отдельного декоратора для каждого типа фильтра и затем использовать разные комбинации фильтров в зависимости от потребностей
