...
Служит для предоставления простого интерфейса в рамках одного объекта для сложной подсистемы или нескольких подсистем (библиотеки или библиотек).


![](../_img/facade.png)

**Пример** - необходимо провести загрузку видео с помощью библиотеки *YouTube* и преобразовать видео с помощью библиотеки *FFMpeg*. Клиентскому коду требуется лишь небольшая часть функциональности этих библиотек. Фасад *YouTubeDownloader* скрывает за собой подсистемы *YouTube* и *FFMpeg* и предоставляет простой интерфейс для нужного функционала
