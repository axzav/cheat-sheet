...
Позволяет использовать в коде класс с неподходящим интерфейсом при помощи создания обертки с нужным интерфейсом.

![](../_img/adapter.png)

**Пример**: Существует система в которой реализован сервис отправки нотификаций, который требует объекты определенного интерфейса. При добавлении *SlackApi* выяснилось, что интерфейс *SlackApi* не подходит для клиентского кода. Для решения проблемы создается адаптер *SlackNotification* который уже обладает нужным интерфейсом
