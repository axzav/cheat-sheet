...
В пхп для реализации совместной многозадачности нужно создать собственный обработчик задач - https://habr.com/ru/post/164173/

Механизм обеспечивается с помощью метода `send` генератора, который позволяет отправлять данные в генератор и получать их из генератора:
