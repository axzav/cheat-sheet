# References, passing values and copy-on-write

## References

При указании ссылки на переменную, обе переменные будут указывать на одну и ту же область в памяти. При изменении значения одной переменной, другая переменная, которая ссылается на ту же переменную, так же будет изменена:
...
При передаче переменной скалярного типа или массива по ссылке, изменение переменной в теле функции также изменит ее значение в родительском скоупе:
...
Механизм `copy-on-write` используется для оптимизации памяти при присвоении переменных или передаче аргумента функции. Когда происходит передача по значению, значение не будет скопировано сразу же и обе переменные будут указывать на одну и ту же область в памяти. Копирование произойдет в момент того, как переменная будет изменена:
...
Скалярные типы (integers, floats, strings, and booleans) всегда передаются по значению, т.е. значение будет скопировано в функцию. Механизм `copy-on-write` будет использован для оптимизации:
